...
class SC_CheckError {
    ...
    // value[5] = MM(終了)
    // value[6] = DD(終了)
    function CHECK_DATES($value) {
        if (isset($this->arrErr["end_year"])) {
            return;
        }
        //開始年月日と終了年月日をそれぞれ連結
        $this->start_year = (int) implode("", array($value[1], $value[2], $value[3]));
        $this->end_year = (int) implode("", array($value[4], $value[5], $value[6]));

        //開始年月日よりも終了年月日が小さい場合エラー
        if ($this->start_year > $this->end_year) {
            $this->arrErr["end_year"] = "※ " . $value[0] . "が正しくありません。<br />";
        }
    }
